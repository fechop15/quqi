...
export function VentaForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const ventaRegistradaData = {
        id: ventaRef.id,
        ...ventaData,
      };
      
      setVentaRegistrada(ventaRegistradaData);
      
      if (generarRecibo) {
        toast.success('Venta registrada - Generando recibo...');
      } else {
  ...
  return (
    <>
      ...
      {/* Modal de Recibo */}
      {ventaRegistrada && generarRecibo && (
        <ReciboModal venta={ventaRegistrada} onClose={() => router.push('/ventas')} />
      )}
    </>
  );
}
